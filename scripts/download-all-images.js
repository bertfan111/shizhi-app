...
async function downloadImage(char, text) {
  // Use the actual story text to generate perfectly matching illustration
  const prompt = `Children book illustration, 2026 sci-fi style, cartoon, soft pastel colors, very bright and cheerful, NO TEXT, NO LETTERS. Story scene: ${text}`;
  const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?nologo=true&seed=${Math.floor(Math.random()*10000)}`;
...
async function main() {
  ...
  const allChars = ds.characters;
  
  console.log(`🚀 开始在后台为全部 ${allChars.length} 个字重新生成完美匹配故事的插画...`);
